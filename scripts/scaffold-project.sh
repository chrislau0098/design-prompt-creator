...
echo ""
echo "Next steps (R-101 workflow — Example-first, Prompt auto-extracted after Chris confirms):"
...
echo "     then sync + launch the renderer to look at the Design Example iframe:"
echo "       ./scripts/sync-to-management.sh --scenario $SCENARIO --style <handle>"
echo "       cd design-prompt-management && bun install && bun dev"
echo "       open http://localhost:5173/?view=design-example&style=<handle>"
echo "     Iterate on the Slot until Chris confirms the Example is OK (this is the HARD GATE)."
echo "  5. After Chris confirms, Cowork dispatches an Opus sub-agent to auto-extract the"
echo "     Design Prompt from the confirmed Slot + Example (see reference/05-prompt-generate.md"
echo "     Step B for the dispatch stub). The Prompt is the frozen artifact the weak model reads."
echo "  6. Final sync to the renderer + review (System ⊆ Prompt ⊆ Slot SoT, ≤ 620 lines):"
echo "       ./scripts/sync-to-management.sh --scenario $SCENARIO --style <handle>"
echo "       (see reference/07-prompt-review.md for the Step 0 confirm gate + three axes)"
